"""
Фильтр токсичности для городского помощника.

Обеспечивает фильтрацию токсичных сообщений пользователей.

Поддерживает два бэкенда:
- regex (по умолчанию): быстрый, без зависимостей, примитивный
- ml: точный, использует 'rubert-tiny-toxicity' модель, может быть медленнее и требует torch/transformers
...
class ToxicityBackend(StrEnum):
    ...
    REGEX = 'regex'  # быстрый, на основе простых паттернов
...
def get_toxicity_filter(backend: ToxicityBackend | str | None = None) -> ToxicityFilter:
    ...
    if _filter_instance is None or _current_backend != backend:
        ...
        if backend == ToxicityBackend.ML:
            # ленивая загрузка для ML-версии (тяжёлые зависимости)
            from langgraph_app.services.toxicity_advanced import ToxicityFilterML
...
def get_toxicity_filter_regex() -> ToxicityFilter:
    """
    Явно возвращает regex-based фильтр
    """
...
def get_toxicity_filter_ml() -> ToxicityFilter:
    """
    Явно возвращает ML-based фильтр
    """
