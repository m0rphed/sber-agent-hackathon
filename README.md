...
│   ├── agent/                  # Агент и классификация
│   ├── api/                    # API эндпоинты
│   ├── graphs.py               # LangGraph графы (supervisor, hybrid, rag)
│   ├── rag/                    # RAG компоненты
│   ├── services/               # Сервисы
...
│   │   ├── bot_main.py
│   │   └── config.py
│   └── max/                    # MAX/VK Teams бот (maxapi)
│       ├── bot_main.py
│       └── config.py
│
├── packages/                   # UV Workspaces (монорепо)
│   ├── langgraph-app/          # LangGraph Server пакет
│   ├── streamlit-ui/           # Streamlit UI пакет
│   ├── bot-telegram/           # Telegram бот пакет
│   └── bot-max/                # MAX бот пакет
│
├── libs/
│   └── shared/                 # Общие утилиты
│
...
├── pyproject.toml              # UV workspace + зависимости
└── uv.lock                     # Lock файл
```
